...
if (name!=''):
    ...
    col2.markdown("The BLEU score compares a sentence against one or more reference sentences and tells how well does the candidate sentence matched the list of reference sentences. It gives an output score between 0 and 1. A BLEU score of 1 means that the candidate sentence perfectly matches one of the reference sentences. ** Please take into account that not of all images have original captions. It means that for some images BLEU score will be equal to 0 ** {: style='text-align: justify'}",unsafe_allow_html=True)
